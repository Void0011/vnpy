...
class StrategyChartOverlay:
    """Provide optional indicator overlays for a specific strategy chart."""
...
class DoubleMaChartOverlay(StrategyChartOverlay):
    """Overlay provider for the built-in double moving average strategy."""
    ...
    def add_items(
        ...
        fast_window: int = int(strategy_setting.get("fast_window", 10))
        slow_window: int = int(strategy_setting.get("slow_window", 20))

        closes: np.ndarray = np.array([bar.close_price for bar in bars], dtype=float)
        x_values: np.ndarray = np.arange(len(closes))

        fast_values: np.ndarray = self._calculate_sma_series(closes, fast_window)
        slow_values: np.ndarray = self._calculate_sma_series(closes, slow_window)

        fast_item: pg.PlotCurveItem = pg.PlotCurveItem(
            x_values,
            fast_values,
            pen=pg.mkPen("#4FC3F7", width=2.8)
        )
        slow_item: pg.PlotCurveItem = pg.PlotCurveItem(
            x_values,
            slow_values,
            pen=pg.mkPen("#FFB74D", width=2.8)
        )

        candle_plot.addItem(fast_item)
        candle_plot.addItem(slow_item)
        return [fast_item, slow_item]

    @staticmethod
    def _calculate_sma_series(values: np.ndarray, window: int) -> np.ndarray:
        result: np.ndarray = np.full(len(values), np.nan)

        if window <= 0 or len(values) < window:
            return result

        weights: np.ndarray = np.ones(window, dtype=float) / window
        sma_values: np.ndarray = np.convolve(values, weights, mode="valid")
        result[window - 1:] = sma_values
        return result


DEFAULT_CHART_OVERLAY: StrategyChartOverlay = StrategyChartOverlay()
STRATEGY_CHART_OVERLAYS: dict[str, StrategyChartOverlay] = {
    "DoubleMaStrategy": DoubleMaChartOverlay(),
}
...
class BacktesterManager(QtWidgets.QWidget):
    ...
    def show_candle_chart(self) -> None:
        ...
        if not self.candle_dialog.is_updated():
            class_name: str = self.class_combo.currentText()
            strategy_setting: dict = self.settings.get(class_name, {})

            history: list = self.backtester_engine.get_history_data()
            self.candle_dialog.update_history(history)
            self.candle_dialog.update_strategy(class_name, strategy_setting)
...
class BacktesterChart(pg.GraphicsLayoutWidget):
    ...
    def set_data(self, df: DataFrame) -> None:
        ...
        count: int = len(df)
        x_values: np.ndarray = np.arange(count)
        balance_values: np.ndarray = df["balance"].to_numpy()
        drawdown_values: np.ndarray = df["drawdown"].to_numpy()
        net_pnl_values: np.ndarray = df["net_pnl"].to_numpy()
        ...
        self.balance_curve.setData(x_values, balance_values)
        self.drawdown_curve.setData(x_values, drawdown_values)
        ...
        hist, x = np.histogram(net_pnl_values, bins="auto")
        x = x[:-1]
...
class CandleChartDialog(QtWidgets.QDialog):
    ...
    def __init__(self) -> None:
        ...
        self.items: list = []
        self.indicator_items: list = []
        self.strategy_class_name: str = ""
        self.strategy_setting: dict = {}
        self.active_overlay: StrategyChartOverlay = DEFAULT_CHART_OVERLAY
    ...
    def init_ui(self) -> None:
        ...
        self.overlay_legend_widget: QtWidgets.QWidget = QtWidgets.QWidget()
        self.overlay_legend: QtWidgets.QHBoxLayout = QtWidgets.QHBoxLayout()
        self.overlay_legend.setContentsMargins(0, 0, 0, 0)
        self.overlay_legend_widget.setLayout(self.overlay_legend)
        self.overlay_legend_widget.hide()
        ...
        vbox.addWidget(self.overlay_legend_widget)
        self.setLayout(vbox)
    ...
    def update_strategy(self, class_name: str, strategy_setting: dict) -> None:
        """"""
        self.strategy_class_name = class_name
        self.strategy_setting = dict(strategy_setting)
        self.active_overlay = STRATEGY_CHART_OVERLAYS.get(class_name, DEFAULT_CHART_OVERLAY)
        self.update_overlay_legend()
        self.update_indicator_overlays()

    def update_overlay_legend(self) -> None:
        """"""
        while self.overlay_legend.count():
            item = self.overlay_legend.takeAt(0)
            widget = item.widget()
            if widget:
                widget.deleteLater()

        entries: list[tuple[str, str]] = self.active_overlay.get_legend_entries()
        if not entries:
            self.overlay_legend_widget.hide()
            return

        self.overlay_legend.addStretch()
        for text, color in entries:
            label: QtWidgets.QLabel = QtWidgets.QLabel(text)
            label.setStyleSheet(f"color:{color}")
            self.overlay_legend.addWidget(label)
            self.overlay_legend.addStretch()

        self.overlay_legend_widget.show()

    def update_indicator_overlays(self) -> None:
        """"""
        ...
        bars: list[BarData] = list(self.ix_bar_map.values())
        self.indicator_items = self.active_overlay.add_items(
            candle_plot,
            bars,
    ...
    def clear_data(self) -> None:
        ...
        self.strategy_setting.clear()
        self.active_overlay = DEFAULT_CHART_OVERLAY
        self.update_overlay_legend()
        self.high_price = 0
        self.low_price = 0
        self.price_range = 0
