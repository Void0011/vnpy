...
行情优先使用 AkShare 的东方财富接口，失败时重试并切换腾讯日线接口；备用接口保留相同的前复权请求，按年份分段下载，并继续执行行情日期校验。运行日志会记录实际行情来源。各数据商的历史修订可能存在差异。

推送提醒相关代码到 `codex/dca-live` 会自动运行一次强制 dry-run，只生成卡片，不发送策略或失败消息、不写成功状态。正式定时运行仍由 `DCA_REMINDER_ENABLED` 控制。
...
提醒任务每次检出所选分支的最新版本，因此重跑旧 Action 时也会读取最新的发送记录，避免因旧提交中的空状态再次推送。主构建仍按触发它的提交进行验证。
